...
#' @param inpath PIHM input directory, default = inpath which is global variable
#' @param outpath PIHM output directory, default = outpath which is global variable
#' @return Character of full path of input files for PIHM
#' @export
PIHM.filein <- function(projname=get('PRJNAME', envir=.pihm),
                        inpath = get('inpath', envir=.pihm), 
                        outpath = get('outpath', envir=.pihm)
                        ){
