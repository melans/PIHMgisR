...
PTF <- function (x, topsoil=TRUE){
  ...
  if(is.matrix(x) || is.data.frame(x)){
    if(ncol(x) !=4){
      warning('Input X must be N*4 matrix or data.frame')
      return(NA);
    }
    y=x;
  }else{
  ...
  for (i in  1:nsoil){
    ...
    # exp in Alpha function is what is used in PIHMgis .
    #Beta
    # if(Alpha <0){
    #   print(Alpha);
    # }
...
PTF.soil <- function(x, topsoil=TRUE){
  ...
  colnames(ret) = c('INDEX', 'KsatV(m_d)', 'ThetaS(m3_m3)', 'ThetaR(m3_m3)', 'InfD(m)', 'Alpha(1_m)', 'Beta', 'hAreaF(m2_m2)', 'macKsatV(m_d)')
  ret[,1:3]= ptf[,1:3]
  ret[,2]= ptf[,2]    #
